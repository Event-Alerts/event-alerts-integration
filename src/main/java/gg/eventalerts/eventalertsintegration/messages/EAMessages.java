...
public class EAMessages extends AnnoyingMessages {
    ...
    public static class Command extends SubConfig<EAMessages> {
        ...
        public static class Linking extends SubConfig<Command> {
            ...
            public static class Check extends SubConfig<Linking> {
                public Check(@org.jetbrains.annotations.NotNull Linking root) {
                    super(root);
                }

                @Comment("Placeholders: %unlinked%")
                @NotNull public JsonChatMessage result = getRoot().getRoot().getRoot().defaultMessage("%prefix%Successfully kicked %s%%unlinked%%p% unlinked players of the %s%%total%%p% online!@@%p%%command%@@%command%");
            }

            public static class Discord extends SubConfig<Linking> {
                public Discord(@org.jetbrains.annotations.NotNull Linking root) {
                    super(root);
                }

                @NotNull public JsonChatMessage not_linked = getRoot().getRoot().getRoot().defaultMessage("%prefix%%se%%input%%pe% is not linked to a Discord account!@@%pe%%command%@@%command%");

                @Comment
                @Comment("Placeholders: %username%, %id%")
                @NotNull public JsonChatMessage linked = getRoot().getRoot().getRoot().defaultMessage(MapGenerator.LINKED_HASH_MAP.mapOf(
                        "suggest_input", "%prefix%%s%%input%%p% is linked to @@%p%%command%@@%command%",
                        "copy_username", "%s%%username%@@%p%%id%@@%username% (%id%)"));
            }

            public static class Minecraft extends SubConfig<Linking> {
                public Minecraft(@org.jetbrains.annotations.NotNull Linking root) {
                    super(root);
                }

                @NotNull public JsonChatMessage not_linked = getRoot().getRoot().getRoot().defaultMessage("%prefix%%se%%input%%pe% is not linked to a Minecraft account!@@%pe%%command%@@%command%");
        ...
        public static class CrossBan extends SubConfig<Command> {
            ...
            public static class Check extends SubConfig<CrossBan> {
                public Check(@org.jetbrains.annotations.NotNull CrossBan root) {
                    super(root);
                }

                @Comment("Placeholders: %banned%")
                @NotNull public JsonChatMessage result = getRoot().getRoot().getRoot().defaultMessage("%prefix%Successfully kicked %s%%banned%%p% banned players!@@%p%%command%@@%command%");
            }
        }

        public static class Transfer extends SubConfig<Command> {
            public Transfer(@org.jetbrains.annotations.NotNull Command root) {
                super(root);
            }

            @Comment("Placeholders: %server%")
            @NotNull public JsonChatMessage transferring = getRoot().getRoot().defaultMessage("%prefix%Transferring you to %s%%server%%p%...@@%p%%command%@@%command%");

            @Comment
            @NotNull public JsonChatMessage disabled = getRoot().getRoot().defaultMessage("%prefix%%pe%Server transferring is only available on 1.20.5 or higher!@@%pe%%command%@@%command%");
        }
    }
}
